...
interface NavbarProps {
    megaMenuBanner: BannerSlide[];
}

const MegaMenuBanner = ({ slides }: { slides: BannerSlide[] }) => {
    const [current, setCurrent] = useState(0);
    const href = slides[current]?.href?.trim() || "";
    ...
    const frame = (
        <div className="relative h-full w-66.5 shrink-0 overflow-hidden rounded-secondary border border-white/30 hover:border-white/40 hover:brightness-85 duration-300">
            {slides.map((slide, i) => (
                <Image
                    key={`${slide.image}-${i}`}
                    src={slide.image}
            ...
            {slides.length > 1 && (
                <div className="absolute bottom-2 left-1/2 z-10 flex -translate-x-1/2 gap-1 rounded-full bg-darkColor/10 p-1 backdrop-blur-xs">
                    {slides.map((_, i) => (
                        <button
                            key={i}
                            type="button"
    ...
    if (!href) {
        return <div className="block h-full">{frame}</div>;
    }
        ...
        >
            <div className="flex gap-3 items-stretch">
                        ...
                        >
                            ...
                            >
                                <SmartImage
                                    ...
                                    src={
                                        item.img ??
                                        "https://images.unsplash.com/photo-1694327875197-252cfe16f668?q=80&w=2232&auto=format&fit=crop"
                                    }
                                    alt={item.title}
                ...
                {bannerSlides.length > 0 && (
                    ...
                    >
                        <MegaMenuBanner slides={bannerSlides} />
                    </div>
                )}
            </div>
        </div>
...
export const Navbar = ({ megaMenuBanner }: NavbarProps) => {
        ...
        services: [
            {
                title: "GO Digital",
                description:
                    "A bundled service combining Website Development and Social Media Management to build a complete digital presence for your business.",
                href: "/go-digital",
                col: "row-span-2",
            ...
            },
            {
                title: "GO Website",
                description:
                    "Professional website development designed to increase credibility and generate more leads for your business.",
            ...
            },
            {
                title: "GO Social",
                description:
                    "End-to-end social media management including content strategy, design, and optimization to grow engagement.",
                href: "/social-media",
                col: "row-span-1",
                icon: Clover,
            },
