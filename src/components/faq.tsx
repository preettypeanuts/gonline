...
export const FaqSection = ({
    faqs,
    title = <>Got Questions? We <span className="text-thirdColor">Answer</span></>,
}: FaqSectionProps) => {
        ...
        <section className="spacing space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 mx-4 md:mx-10">
                ...
                <Accordion type="single" collapsible>
                    {faqs.slice(0, half).map((item, idx) => (
                ...
                {/* RIGHT COLUMN */}
                <Accordion type="single" collapsible>
                    {faqs.slice(half).map((item, idx) => (
                        <FaqAccordionItem key={idx} item={item} index={idx + half} />
                    ))}
                </Accordion>
...
function FaqAccordionItem({
    ...
    return (
        <AccordionItem value={`item-${index}`} className="border-b-0 mb-2">
            <AccordionTrigger
                className="
            ...
            <AccordionContent
                className="
                    ...
                    rounded-b-xl
                    border-t border-neutral-700 dark:border-neutral-200
                "
            >
                {item.answer}
            </AccordionContent>
